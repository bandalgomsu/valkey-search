...
- `VSIM <field> <vector>` (required): The vector arm. `<field>` is a declared vector attribute and `<vector>` is a binary blob, supplied through `PARAMS`.
  - `KNN <count> [K <k>] [EF_RUNTIME <ef>] [SHARD_K_RATIO <ratio>]` (optional): The vector search parameters. `<count>` is a count of the arguments that follow within the block, not a count of parameters. `K` is the number of nearest neighbors to retrieve, between 1 and 10000, and defaults to 10. Omitting the whole block, or writing `KNN 0`, is the same as taking every default. `EF_RUNTIME` tunes HNSW's search breadth. `SHARD_K_RATIO` is a number greater than 0 and at most 1. In cluster mode it reduces how many results each shard returns: each shard is asked for `max(ceil(base / shards), ceil(base * ratio))`, where `base` is the smaller of `K` and the `COMBINE` window. The `ceil(base / shards)` term is a floor: a ratio below `1 / shards` has no further effect. This trades recall for less work: if the nearest neighbors are concentrated on a few shards, the arm can return fewer or different results. It has no effect on a standalone server, and without it every shard is asked for `K`.
