...
| 1.3.0   | `compatibility-ft_info_score_field` | The `FT.INFO` `index_definition` block is a 6-element array which reports `default_score` as the bulk string `"1"` regardless of the index's configured `SCORE`, and does not report `SCORE_FIELD` at all. | The block is an 8-element array: `default_score` reports the index's configured `SCORE` as a double, and a new `score_field` pair reports the configured `SCORE_FIELD` (an empty string when unset). |
| 1.3.0   | `compatibility-aggregate_json_field_names` | On a JSON index, `FT.AGGREGATE` names each output column after the schema identifier, so a field declared `$.price AS price` comes back as `$.price` from every stage (`LOAD`, `APPLY`, `FILTER`, `GROUPBY`, `SORTBY`). | The column is named by the field token as the query wrote it, matching Redisearch: `@price` comes back as `price`, and a JSON path `$.price` comes back as `$.price`. HASH indexes are unaffected, since there the identifier and the attribute name are always identical. |
| 1.3.0   | `compatibility-ft_aggregate_load_as` | `AS` is treated as an ordinary field name in an `FT.AGGREGATE` `LOAD` clause. | `FT.AGGREGATE` `LOAD` honors the `AS <alias>` rename clause. A `LOAD` clause that produces the same output name twice is rejected; loading the same field twice under different output names yields a column per name. |
| 1.3.0   | `compatibility-aggregate_nil_alias_named`<br>`compatibility-expr_missing_propagates`<br>`compatibility-apply_drops_missing_field` | In an `FT.AGGREGATE` pipeline, a reference to a field the key does not have is indistinguishable from an expression that ran and produced nothing: both are left out of the reply, and the record is kept either way. An operator or function given such a reference substitutes a result of its own rather than passing the absence on, so `!(@absent)` evaluates to true and `(@absent)+(1)` yields a value the reply then drops. | The two are distinguished, matching Redisearch. A reference to an absent field propagates through the operators and functions that receive it, and `APPLY` drops any record whose expression referenced one. A value that merely evaluated to nothing — `lower()` of a number, say — is named in the reply with a nil rather than omitted; only a genuinely absent field is omitted. `exists()` is unaffected: reporting on an absent field is its purpose, so it receives the reference rather than propagating it. `&&` is also unaffected, measured against Redis 8: a missing operand there is falsy, so the record is kept and the alias replies `0`. |
| 1.3.0   | `compatibility-reduce_minmax_numeric` | `FT.AGGREGATE` `REDUCE MIN` and `REDUCE MAX` compare their input as it arrives, so a `TAG` or text field is compared as a string and returns one — `MIN` over `apple` and `Banana` answers `Banana`, by byte order. A group in which every input was absent omits the reducer's column from the reply. | `MIN` and `MAX` are numeric, matching Redisearch. An input that is not a number is not folded at all, and a group that folded no value answers the reducer's identity — `inf` for `MIN`, `-inf` for `MAX` — rather than omitting its column. |
| 1.3.0   | `compatibility-ft_aggregate_divide_by_zero` | Every `FT.AGGREGATE` `APPLY` division by zero evaluates to `NaN`. | Division by zero follows IEEE 754, matching Redisearch: a positive numerator yields `+inf`, a negative numerator yields `-inf`, and `0/0` yields `NaN`. |
| 1.3.0   | `compatibility-ft_aggregate_pow_precedence` | In `FT.AGGREGATE` expressions `^` binds at the same precedence as `*` and `/` and is left-associative, so `@a*@b^@c` parses as `(@a*@b)^@c`. | `^` binds more tightly than `*` and `/` and is right-associative, matching Redisearch: `@a*@b^@c` parses as `@a*(@b^@c)` and `@a^@b^@c` as `@a^(@b^@c)`. |
| 1.3.0   | `compatibility-ft_aggregate_logical_precedence` | In `FT.AGGREGATE` expressions `&&` and `\|\|` bind at the same precedence and are left-associative, so `@a\|\|@b&&@c` parses as `(@a\|\|@b)&&@c`. | `&&` binds more tightly than `\|\|`, matching Redisearch and the usual C/SQL precedence: `@a\|\|@b&&@c` parses as `@a\|\|(@b&&@c)`. |
| 1.3.0   | `compatibility-ft_search_apostrophe_phrase` | An unescaped `'` in an `FT.SEARCH` text query is treated as ordinary punctuation and dropped, so `great'wall` parses as the two terms `great` and `wall`. | An unescaped `'` is a phrase delimiter equivalent to `"`, paired left-to-right, matching Redisearch: `'great wall'` is an exact phrase. An apostrophe with no matching close ahead of it is still treated as a separator. |
| 1.3.0   | `compatibility-empty_string_not_numeric` | An empty string coerces to the number `0` in an `FT.AGGREGATE` expression, so `abs("")` is `0`, `timefmt("")` is a formatted epoch and `(0)==("")` is true. | An empty string is not numeric, matching Redisearch: `abs("")` is `nan`, `timefmt("")` is nil and `(0)==("")` is false. |
| 1.3.0   | `compatibility-reduce_sum_avg_empty_nan` | `FT.AGGREGATE` `REDUCE SUM` and `REDUCE AVG` over a group that folded no value at all answer `0`, which is indistinguishable from a group whose values really did sum to zero. | Such a group answers `nan`, matching Redisearch. |
| 1.3.0   | `compatibility-sortby_max_follows_limit` | `FT.AGGREGATE` `SORTBY` keeps at most its own `MAX`, defaulting to 10, no matter what follows it. A later `LIMIT` can only narrow that: `SORTBY @n1 ASC LIMIT 0 1000` returns 10 rows, and `LIMIT 500 100` returns none at all, because the offset falls past the ten records that survived. `MAX 0` returns nothing rather than being treated as absent. | The bound is taken from an adjacent `LIMIT`, matching Redisearch. A `LIMIT` immediately before or after the `SORTBY` raises it to that clause's offset plus count and outranks a smaller `MAX`. A `LIMIT` separated from the `SORTBY` by any other stage, such as `GROUPBY` or `APPLY`, does not affect it. `MAX 0` means no `MAX`, so the default of 10 applies, and `SORTBY` with neither still keeps 10. |
| 1.3.0   | `compatibility-ft_search_withscores_double` | The `FT.SEARCH` `WITHSCORES` score is replied as a bulk string formatted with 12 significant digits. | The score is replied as a double, matching Redisearch: the shortest round-trip string under RESP2 and a native double under RESP3. |
| 1.3.0   | `compatibility-ft_search_sortkey_vec_score_precision` | When `FT.SEARCH` sorts by the KNN distance, the `WITHSORTKEYS` sort key is formatted with 12 significant digits, e.g. `#0.10000000149`. | The sort key is formatted with `%.17g`, matching Redisearch, e.g. `#0.10000000149011612`. |
